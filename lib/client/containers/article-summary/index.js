...
const ArticleSummaryContainer = React.createClass({
  ...
  statics: {
    fetchData (store, params) {
      return store.dispatch(changeArticle(params.title, false))
    }
  },
  componentDidMount () {
    const {dispatch, params} = this.props
    dispatch(changeArticle(params.title, false))
  },
...
function selectArticle ({selectedArticle, articles}) {
  return {
    lead: articles[selectedArticle.title] && articles[selectedArticle.title].summary
  }
}
