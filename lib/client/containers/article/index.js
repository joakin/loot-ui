...
import { changeArticle, expandArticle, toggleSection } from '../../actions'
...
const ArticleContainer = React.createClass({
  ...
  render () {
    let {article, history, params, expandArticle, toggleSection} = this.props
    return (!article ? null
      : <CaptureClicks history={history}>
          <Article {...article}
            onExpand={() => expandArticle(params.title)}
            onRetry={() => this.loadArticle(params.title)}
            onHeadingClick={(index) => toggleSection(params.title, index)}/>
        </CaptureClicks>)
  },
  statics: {
...
function selectActions (dispatch) {
  return bindActionCreators({ changeArticle, expandArticle, toggleSection }, dispatch)
}
